...
EXPDIR=/scratch/gpfs/awettig/delve/k-means-clustering/exps/dclm-1level-k24/level1
cd /scratch/gpfs/awettig/delve/k-means-clustering/scripts

master_addr=$(scontrol show hostnames "$SLURM_JOB_NODELIST" | head -n 1)

PYTHONPATH=.. \
srun -N 3 --unbuffered --output="$EXPDIR"/logs/%j_%t_log.out --error="$EXPDIR"/logs/%j_%t_log.err  torchrun \
  --nnodes=3 \
  --nproc_per_node=8 \
  --rdzv_backend=c10d \
