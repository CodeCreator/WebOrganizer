...
def load_model_and_tokenizer(options):
    ...
    model = AutoModelForSequenceClassification.from_pretrained(options.model_name, trust_remote_code=True)
...
def predict_fn(subset, indices, process_id, options):
    ...
    for batch in tqdm(data_loader, disable=(process_id != 0)):
        ...
        logits = model_output.logits.float().cpu()
        choices = logits.argmax(axis=-1).cpu()

        for seq_logits, seq_choice in zip(logits, choices):
            yield {"logits": seq_logits.numpy(), "choice": seq_choice.item()}
